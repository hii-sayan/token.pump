...
const { Faculty_Glyphic } = require("next/font/google")
...
describe("Factory", function () {
    ...
    async function deployFactoryFixture() {
        ...
        //Create token
        const transaction = await factory.connect(creator).create("sayan", "syn", { value: FEE })
    ...
    describe("Creating", function (){
        ...
        it("Slould set the total supply",async function(){
            const { factory, token } = await loadFixture(deployFactoryFixture)
            const totalSupply = ethers.parseUnits("1000000", 18)
            expect(await token.balanceOf(await factory.getAddress())).to.equal(totalSupply)
        })

        it("Slould update teh ETH balance",async function(){
            const { factory } = await loadFixture(deployFactoryFixture)
        ...
        it("Should create the sale", async function () {
            ...
            expect(sale.sold).to.equal(0n)
            expect(sale.raised).to.equal(0n)
    ...
    describe("Buying", function(){
        ...
        //Check teh buyer received tokens
